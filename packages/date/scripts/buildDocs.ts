      ...
      readme = readme
        ...
        .replace(
          ...
          `$1\n${
            briefList.map(
              brief => {
                const sourceUrl = `https://github.com/fjc0k/vtils/blob/master/packages/date/src/${brief.source.fileName}#L${brief.source.line}`
                const apiUrl = (
                  Number(kind) === ReflectionKind.Class
                    ? `https://fjc0k.github.io/vtils/date/classes/${brief.name.toLowerCase()}.html`
                    : `https://fjc0k.github.io/vtils/date/globals.html#${brief.name.toLowerCase()}`
                )
                return dedent`
                  #### ${brief.name}
